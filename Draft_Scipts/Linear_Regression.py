...
def train_one_epoch(model, optimizer, data_loader,global_step, global_step_loss):
    ...
    for train_features, normalized_target_sales in data_loader:
        ...
        for i in range(0, len(scale_target_sales.tolist())):
            totalcount = 0
            workingcount = 0
            percent_within_lower_bound = (confidence) * scale_target_sales.tolist()[i]
            percent_within_upper_bound = (2 - confidence) * scale_target_sales.tolist()[i]
...
if __name__ == '__main__':
    ...
    for k in range (0, epochs):
        ...
        for i in range(0, len(predictions_validation_iteration)):
            ...
            percent_set_iteration.append(abs(100 * (differance / validation_sales.tolist()[i])))   #these absolute differances will be more because of no normalization
...
#we save the zip file here using the below code
    Path_model = os.path.join(model_path, f"exp{exp_number}_Model_Path.pkl")

    torch.save(model.state_dict(), Path_model)



    pickle.dump(model.to('cpu'), open(Path_model, 'wb'))
#     with open(Path_model, 'wb') as file:
#         pickle.dump(model, Path_model)
